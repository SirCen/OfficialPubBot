...
//  guildMember.guild.add.roles('Test');
// });
    ...
    for(let i = 0; i < length; i++) {
      if (stringArray[i] === "I'm" | stringArray[i] === "Im" | stringArray[i] === "im") {
        ...
        if (found && newMessage !== " Pub Bot") {
          message.channel.send("Hi" + newMessage + ", I'm Pub Bot");
        }
      } else if (!found && (stringArray[i] === "yurr" | stringArray[i] === "Yurr")) {
        message.channel.send('I agree with the above statement');
      }
    }
  }
});
...
client.on('message', message => {
  if(!commandUsed && message.author.bot){
    ...
    if (command === `hot`) {
      ...
      if (rand === 0) {
        message.channel.send(newMessage + ' is HOT!');
      }else if (rand === 1) {
       message.channel.send(newMessage + ' is NOT HOT!');
      }
    }
  }
  commandUsed = true;
